...
from Allignment import AllignImage, OrbAllignAll, SaveAllignmentMatrix, ReadAllignmentMatrix,allignmentMatrixTostring
from micasense import imageutils
from micasense.imageutils import Bounds
import numpy as np


def read_image_from_filelist(imgs):
    im = []
    maxx ,maxy = 0,0
    camera_matrix = []
    for img in imgs:
        tmpim = tiff.imread(img)
        if tmpim.shape[1] == 2616:
            camera_matrix.append(np.array([[7.2*(2592/5.7),0,2592/2],[0,7.2*(1944/4.3),1944/2],[0,0,1]])) #RGB
        else:
            camera_matrix.append(np.array([[6.0*(2592/4.8),0,2592/2],[0,6.0*(1944/3.6),1944/2],[0,0,1]])) #mono
    ...
    return im , camera_matrix , distortion_coof

def AllignImageOtherCam(imgs,allignmat):
    im, camera_matrix , distortion_coof = read_image_from_filelist(imgs)
    ...
    for i in range(im_aligned.shape[2]):
        im_aligned[:,:,i] = cv2.normalize(im_aligned[:,:,i],None,0.,1.,cv2.NORM_MINMAX,cv2.CV_32F)
    return im_aligned

def GetAllignmentMatrixFromOtherCam(imgs):
    im,_, _ = read_image_from_filelist(imgs)
...
def main():
    ...
    camera_matrix = [np.array([[7.2*(2592/5.7),0,2592/2],[0,7.2*(1944/4.3),1944/2],[0,0,1]]) for f in range(3)]
    camera_matrix_mono = [np.array([[6.0*(2592/4.8),0,2592/2],[0,6.0*(1944/3.6),1944/2],[0,0,1]]) for f in range(4)]
    for m in camera_matrix:
        camera_matrix_mono.append(m)
